window.onload=function(){
	...
	(function tab_bond(){
		...
		var aCon = oBond.getElementsByClassName('content');
		for (var i = 0; i < aLi.length; i++) {
			aLi[i].index = i;
			aLi[i].onmouseover = function () {
	...
	function tab(parent,option,con){
		...
		var aCon = oCtab.getElementsByClassName(con);
		var aLi = oOption.getElementsByTagName('li');
		for (var i = 0; i < aLi.length; i++) {
			aLi[i].index = i;
			aLi[i].onmouseover = function () {
				for (var j = 0; j < aLi.length; j++) {
	...
	// var aCon = oWeibo.getElementsByClassName('content');
